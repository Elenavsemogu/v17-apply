...
Сессия пишется на диск `/data` (`bot_data.pickle`): если человек ушёл и вернулся, `/start` предлагает Continue, а не сбрасывает анкету. Other открывает поле ввода сразу.
