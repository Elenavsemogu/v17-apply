...
4. В Apps Script: выбрать функцию `setupTelegramPolling` → Выполнить (одноразово;
   ставит таймеры раз в минуту: кнопки в Telegram **и** отказ из Notion → чат).
...
5. Когда обновляешь код из репозитория — **не затирай** `TELEGRAM_TOKEN` / Notion
   в CONFIG. Если уже затёрла: История версий (часики слева) → старая копия →
   верни четыре поля. После первого успешного запуска они живут в свойствах
   скрипта и пустой CONFIG из git их больше не сотрёт.

**Отказ в две стороны**
- Telegram → Notion: кнопки «Отказ …» шлют письмо и ставят в карточке `Declined`.
- Notion → Telegram: если статус в CRM сменили на **Declined** (или Rejected / Отказ),
  раз в минуту бот снимает кнопки под заявкой и пишет в чат «Отказ в Notion».
  Письмо при отказе из Notion **не** уходит — в CRM могли отказать без шаблона.
  После обновления скрипта ещё раз выполнить `setupTelegramPolling`.
...
- Отказ, поставленный в Notion, доезжает до Telegram с той же задержкой (до минуты).
